document.addEventListener("DOMContentLoaded", () => {
  ...
  function createSeatButtons(seats) {
    ...
    seats.forEach(s => {
      ...
      b.addEventListener("click", function(){
        ...
        this.classList.add("active");
        sendToFiveM("action",{action:this.getAttribute("data-action"),seat:Number(this.getAttribute("data-seat"))});
      });
    });
    initTooltips();
  ...
  seatsGrid.addEventListener("scroll", () => {
    scrollIndicator.classList.toggle("visible", seatsGrid.scrollTop <= 10 && seatsGrid.scrollHeight > seatsGrid.clientHeight);
  });
  ...
  function applyTheme(theme) {
    ...
    switch(theme){
      case "blue": currentThemeColor="#3b82f6";break;
      case "red": currentThemeColor="#ef4444";break;
      case "purple": currentThemeColor="#8b5cf6";break;
      default: currentThemeColor="#4ade80";
    }
    document.querySelectorAll(".active:not(.tab):not(.theme-option)").forEach(el=>el.style.backgroundColor=currentThemeColor);
  ...
  function updateVehicleSpecificOptions(data) {
    ...
    ["fl","fr","rl","rr"].forEach(k =>
      updateButtonVisibility(`door_${k}`, !isBike && data.doors[k])
    );
    updateButtonVisibility("hood",  !isBike && data.doors.hood);
    updateButtonVisibility("trunk", !isBike && data.doors.trunk);
  
    ["fl","fr","rl","rr"].forEach(k =>
      updateButtonVisibility(`window_${k}`, !isBike && data.windows[k])
    );
  ...
  window.addEventListener("message", e => {
    ...
    if (d.type==="settings") {
      if (d.settings.theme) {
        ...
        if(opt){opt.classList.add("active");applyTheme(d.settings.theme);}
      }
      if (d.settings.scale) {
